...
int net_connect(int sockfd, struct sockaddr *addr, int addrlen) 
{
	...
	int (*sys_connect)(struct thread * td, struct sys_connect_args * uap) = (void *)(kernbase + __sys_connect);
...
int net_send(int fd, const void *buf, uint64_t len) 
{
	...
	int (*sys_write)(struct thread * td, struct sys_write_args * uap) = (void *)(kernbase + __sys_send);
